...
public class Ignorable {
  ...
  Set<String> ignoreOututAnnotation = new HashSet<String>(Arrays.asList("JsonSchemaOutputModelIgnore"));
  ...
  public void ignorableJsonSchemaOutputModelIgnoreConfigured() throws Exception {
    ObjectMapper objectMapper = new ObjectMapper();
    ObjectWriter writer = objectMapper.writerWithDefaultPrettyPrinter();
    JsonSchemaGenerator jsonSchemaGenerator = new JsonSchemaGenerator(objectMapper, JsonSchemaConfig.getVanillaJsonSchemaDraft4(), false, ignoreOututAnnotation);
